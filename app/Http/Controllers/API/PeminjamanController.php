    ...
    public function store(StorePeminjamanRequest $request): JsonResponse
    {
        try {
            ...
            return response()->json([
                ...
                // Dioptimalkan menggunakan Resource
            ], 201);
        } catch (Exception $e) {
            return response()->json([
    ...
    public function update(
        ...
        try {
            DB::transaction(function () use ($request, $peminjaman) {
                ...
                foreach ($request->items as $item) {
                    // Ditambahkan lockForUpdate agar konsisten
                    // aman dari race condition saat update data draft
                    $alat = Alat::lockForUpdate()
                        ->findOrFail($item['alat_id']);
        ...
        try {
            DB::transaction(function () use ($peminjaman) {
                ...
                foreach ($peminjaman->detailPinjam as $detail) {
                    // Mengunci baris alat demi validasi final
